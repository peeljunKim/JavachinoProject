...
@Repository
public interface AdminAccomodationFacDAO extends JpaRepository<AccomodationFac, Integer> {
	//페이징
		...
		public List<AccomodationFac> selectAll(int start, int end);
...
//		
		...
		//추가
		@Modifying
	    @Query(value = "INSERT INTO accomodation_fac (activity_no, activity_name, activity_addr, activity_explanation, activity_category, activity_price, activity_time, activity_fname1, activity_fname2, activity_fname3) VALUES (ACTIVITY_SEQ.NEXTVAL, :#{#a.activityName}, :#{#a.activityAddr}, :#{#a.activityExplanation}, :#{#a.activityCategory.ordinal()}, :#{#a.activityPrice}, :#{#a.activityTime}, :#{#a.activityFname1}, :#{#a.activityFname2}, :#{#a.activityFname3})", nativeQuery = true)
	    @Transactional
